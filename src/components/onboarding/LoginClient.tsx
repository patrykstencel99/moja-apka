...
export function LoginClient() {
  ...
  const loadStatus = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setHasUsers(true);
        setStatusError('Nie udalo sie pobrac statusu instalacji. Dostepny jest tryb logowania.');
        return;
      }
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setError('Nie udalo sie uruchomic sesji. Sprawdz polaczenie i sprobuj ponownie.');
      setIsLoading(false);
    }
  };
  ...
  return (
    <section className="panel auth-panel">
      ...
      {statusError && (
        <Banner tone="warning" title="Status instalacji">
          {statusError}
        </Banner>
      )}
      ...
      <div className="stack">
        ...
        <Button
          ...
          disabled={
            ...
            (hasUsers === false &&
              (confirmPassword.trim().length < 8 || confirmPassword.trim() !== password.trim()))
          }
        >
          {isLoading ? (hasUsers ? 'Logowanie...' : 'Tworzenie konta...') : hasUsers ? 'Zaloguj sie' : 'Utworz pierwsze konto'}
        </Button>
        <small>
          {hasUsers === null
            ...
            : hasUsers
              ...
              : confirmPassword.trim() && confirmPassword.trim() !== password.trim()
